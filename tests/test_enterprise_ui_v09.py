...
def test_enterprise_console_is_a_dedicated_operational_surface(client):
    ...
    assert "数据保留" in html
    assert "人工复核" in html
    assert "/static/enterprise.js?rev=classic-20260904" in html

    script = client.get("/static/enterprise.js?rev=classic-20260904")
    ...
    stylesheet = client.get("/static/enterprise.css?rev=classic-20260904")
    assert stylesheet.status_code == 200
    assert ".app-shell { display: grid; grid-template-columns: 224px minmax(0, 1fr);" in stylesheet.text


def test_main_workbench_bootstraps_identity_and_propagates_tenant_context(client):
    page = client.get("/")
    script = client.get("/static/app.js?rev=classic-20260904")

    assert page.status_code == 200
    assert 'id="workbenchOrganization"' in page.text
    assert 'id="workbenchLogin"' in page.text
    assert script.status_code == 200
    assert 'api("/api/v1/me", {organizationScoped: false})' in script.text
    assert 'headers["X-AI-Examiner-Organization"] = state.organizationId' in script.text
    assert 'credentials: "same-origin"' in script.text
    assert "bootstrapWorkbenchIdentity().then" in script.text
    assert "provider.id === preferredProvider?.id" in script.text
    assert 'provider.provider === "mock" ? "checked"' not in script.text
